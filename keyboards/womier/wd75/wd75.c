...
void wireless_devs_change_kb(uint8_t old_devs, uint8_t new_devs, bool reset) {
    ...
    switch (new_devs) {
        ...
        case DEVS_2G4: {
            if (reset) {
                rgb_matrix_wls_indicator_set(19, (RGB){RGB_BLUE}, 200, 1);
            } else {
                rgb_matrix_wls_indicator_set(19, (RGB){RGB_BLUE}, 500, 1);
            }
        } break;
        default:
            break;
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (host_keyboard_led_state().caps_lock) {
        rgb_matrix_set_color(45, 0x77, 0x77, 0x77);
    }
    if(keymap_config.no_gui){
        rgb_matrix_set_color(76, 0x77, 0x77, 0x77);
    }
#    ifdef WIRELESS_ENABLE
